...
let reviews_http = `https://api.themoviedb.org/3/movie/${movie_id}/reviews?api_key=6483a6e211092b09b7466883e175cd26&language=en-US&page=1`

fetch(`${reviews_http}`)
.then(res => res.json())
.then(data => {
    const idk = document.getElementById('reviews');
    for(let i =0; i < 5; i++){
        idk.innerHTML += `
        <div class="author">
        <h1 class="movie-name">${data.results[i].author}</h1>
        <p>${data.results[i].content.substring(0, 200) + '...'}</p>
        <hr>
        <br>
    </div>`
    }
